/*
 * netlink/genl_ctrl.c		Generic Netlink Controller
 */
...
static int ctrl_msg_parser(struct nl_cache_ops *ops, struct genl_cmd *cmd,
  ...
  if (info->attrs[CTRL_ATTR_MCAST_GROUPS]) {
    ...
    nla_for_each_nested(nla, nla_grps, remaining) {
      ...
      err =
          nla_parse_nested(tb, CTRL_ATTR_MCAST_GRP_MAX, nla, family_grp_policy);
      if (err < 0)
        goto errout;
...
static void __init ctrl_init(void) { genl_register(&genl_ctrl_ops); }

static void __exit ctrl_exit(void) { genl_unregister(&genl_ctrl_ops); }
