/*
 * "Staging" code before porting it to Python. Lists network interfaces on the
 * host through libnl (netlink).
 *
 * I don't know C so it was hard to get a Python application to talk to the
 * kernel by looking at C code, trying to resolve header files and variable
 * origins/values in my head.
 *
 * I'm "staging" my code here in C and once it's working I'll port it over to
 * Python. Here I can insert printf statements to learn what the values of some
 * variables are, and so on.
 *
 ...
 *
 * Build and execute:
 *      gcc -o nl netlink.c $(pkg-config --cflags --libs libnl-genl-3.0) &&
 * ./program
 *
 * Resources:
...
static int callback(struct nl_msg *msg, void *arg) {
  ...
  while (RTA_OK(hdr, remaining)) {
    ...
    if (hdr->rta_type == IFLA_LINKINFO) {
      printf("Found network interface AAA %d: %s\n", iface->ifi_index,
             (char *)RTA_DATA(hdr));
    }
...
int main() {
  ...
  struct nl_sock *socket =
      nl_socket_alloc();             // Allocate new netlink socket in memory.
  ...
  // struct nlmsghdr *nlh;
  // msg = nlmsg_alloc();
  // nlh = nlmsg_put(msg, NL_AUTO_PID, NL_AUTO_SEQ, RTM_GETLINK, sizeof(ifih),
