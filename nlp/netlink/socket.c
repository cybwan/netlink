/*
 * netlink/socket.c		Netlink Socket
 */
...
static int noop_seq_check(struct nl_msg *msg, void *arg) { return NL_OK; }
...
int nl_socket_recv_pktinfo(struct nl_sock *sk, int state) {
  ...
  err =
      setsockopt(sk->s_fd, SOL_NETLINK, NETLINK_PKTINFO, &state, sizeof(state));
  if (err < 0)
    return -nl_syserr2nlerr(errno);
