...
struct nl_msg *unl_genl_msg(struct unl *unl, int cmd, bool dump) {
  ...
  genlmsg_put(msg, NL_AUTO_PID, NL_AUTO_SEQ, genl_family_get_id(unl->family), 0,
              flags, cmd, 0);
...
int unl_request(struct unl *unl, struct nl_msg *msg, unl_cb handler,
                void *arg) {
  struct nl_cb *cb;
  int err;
...
int unl_genl_multicast_id(struct unl *unl, const char *name) {
  ...
  nla_for_each_nested(group, groups, rem) {
    ...
    nla_parse(tb, CTRL_ATTR_MCAST_GRP_MAX, nla_data(group), nla_len(group),
              NULL);
